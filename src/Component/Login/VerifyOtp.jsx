...
const VerifyOtp = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // const result = await axios.post('http://localhost:5000/verify-otp/api', otpData);
      // setOtpResponse(result.data);

      const { authToken } = mock-token-12345;
  ...
  const resendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      // const resendResult = await axios.post('http://localhost:5000/kawadiwala/resend-otp', otpData);
